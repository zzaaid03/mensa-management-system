...
function Admin() {
  ...
  const updateReservStatus = async (resId, newStatus) => {
    try {
      await api.patch(`/admin/reservations/${resId}/status`, {
        status: newStatus,
      });
      fetchData();
    } catch (err) {
  ...
  const handleMealSubmit = async (e) => {
    ...
    const payload = {
      ...
      is_available_tomorrow: mealForm.is_available_tomorrow,
    };
    try {
  ...
  const deleteMeal = async (mealId) => {
    ...
    try {
      ...
      alert(err.response?.data?.detail || "Failed to delete meal");
    }
  };

  const toggleMealAvail = async (meal) => {
    try {
      await api.put(`/admin/meals/${meal.id}`, {
        is_available: !meal.is_available,
      });
      fetchData();
    } catch (err) {
  ...
  const handleTableSubmit = async (e) => {
    ...
    try {
      ...
      const detail = err.response?.data?.detail;
      setTableError(
        typeof detail === "string" ? detail : "Failed to save table",
      );
    }
  };
  ...
  const toggleTableAvail = async (table) => {
    try {
      await api.put(`/admin/tables/${table.id}`, {
        is_available: !table.is_available,
      });
      fetchData();
    } catch (err) {
  ...
  return (
    <div className="container py-8">
      ...
      <p className="section-subtitle">
        Accept orders, manage reservations, add meals, and configure tables.
      </p>
              ...
              {orders.map((o) => (
                <div key={o.id} className={styles.card}>
                  <div className={styles.cardHeader}>
                    <strong>#{o.id}</strong> {o.user?.full_name}{" "}
                    ...
                    <div className={styles.items}>
                      {(o.items || []).map((it) => (
                        <span key={it.id} className={styles.item}>
                          {it.meal?.name || "Meal"} x{it.quantity}
                        </span>
                      ))}
                    </div>
                    <div className={styles.meta}>
                      <span>Pickup: {fmt(o.pickup_time)}</span>
            ...
            <div className={styles.list}>
              {reservations.map((r) => (
                <div key={r.id} className={styles.card}>
                  <div className={styles.cardHeader}>
                    <strong>#{r.id}</strong> {r.user?.full_name}{" "}
                    {statusBadge(r.status, "reservation")}
                  </div>
                  <div className={styles.cardBody}>
                    <div className={styles.meta}>
                      <span>
                        Table {r.table?.table_number} ({r.table?.location})
                      </span>
                      <span>{r.number_of_people} guests</span>
                      <span>{fmt(r.reservation_time)}</span>
                    </div>
                  </div>
                  <div className={styles.cardActions}>
                    <select
                      className={styles.statusSelect}
                      value={r.status}
                      onChange={(e) => updateReservStatus(r.id, e.target.value)}
                    >
      ...
      {activeTab === "meals" && (
        <div className={styles.tabContent}>
          ...
          >
            ...
            {mealSuccess && (
              <div className={styles.okBanner}>{mealSuccess}</div>
            )}
            ...
            <div className={styles.formRow}>
              <input
                ...
                onChange={(e) =>
                  setMealForm((p) => ({ ...p, calories: e.target.value }))
                }
              />
              <input
                className={styles.input}
                ...
                onChange={(e) =>
                  setMealForm((p) => ({ ...p, protein: e.target.value }))
                }
              />
              <input
                className={styles.input}
                ...
                onChange={(e) =>
                  setMealForm((p) => ({ ...p, carbs: e.target.value }))
                }
              />
              <input
                className={styles.input}
                ...
                value={mealForm.fat}
                onChange={(e) =>
                  setMealForm((p) => ({ ...p, fat: e.target.value }))
                }
              />
            </div>
            <input
              ...
              onChange={(e) =>
                setMealForm((p) => ({ ...p, allergens: e.target.value }))
              }
            />
            <label className={styles.checkbox}>
              <input
                ...
                onChange={(e) =>
                  setMealForm((p) => ({ ...p, is_available: e.target.checked }))
                }
              />{" "}
              Available Today
            </label>
            <label className={styles.checkbox}>
              <input
                type="checkbox"
                checked={mealForm.is_available_tomorrow}
                onChange={(e) =>
                  setMealForm((p) => ({
                    ...p,
                    is_available_tomorrow: e.target.checked,
                  }))
                }
              />{" "}
              Available Tomorrow
            ...
            <div className={styles.formActions}>
              <button type="submit" className={styles.saveBtn}>
                {mealForm.editingId ? "Update Meal" : "Add Meal"}
              </button>
              {mealForm.editingId && (
                <button
                  type="button"
                  className={styles.cancelBtn}
                  onClick={() => setMealForm(resetMealForm())}
                >
                  Cancel
                </button>
              )}
          ...
          <div className={styles.list} style={{ marginTop: 24 }}>
            {meals.map((m) => (
              <div
                key={m.id}
                className={`${styles.card} ${!m.is_available ? styles.cardDisabled : ""}`}
              >
                <div className={styles.cardHeader}>
                ...
                <div className={styles.meta}>
                  ...
                  <span
                    className={
                      m.is_available ? styles.availYes : styles.availNo
                    }
                  >
      ...
      {activeTab === "tables" && (
        <div className={styles.tabContent}>
          ...
          >
            ...
            {tableSuccess && (
              <div className={styles.okBanner}>{tableSuccess}</div>
            )}
            <div className={styles.formRow}>
              <input
                className={styles.input}
                ...
                onChange={(e) =>
                  setTableForm((p) => ({ ...p, table_number: e.target.value }))
                }
                required
              />
              <input
              ...
              <select
                ...
                onChange={(e) =>
                  setTableForm((p) => ({ ...p, location: e.target.value }))
                }
              >
                {LOCATIONS.map((l) => (
            ...
            <div className={styles.formActions}>
              <button type="submit" className={styles.saveBtn}>
                {tableForm.editingId ? "Update Table" : "Add Table"}
              </button>
              {tableForm.editingId && (
                <button
                  type="button"
                  className={styles.cancelBtn}
                  onClick={() => setTableForm(resetTableForm())}
                >
                  Cancel
                </button>
              )}
          ...
          <div className={styles.list} style={{ marginTop: 24 }}>
            {tables.map((t) => (
              <div
                key={t.id}
                className={`${styles.card} ${!t.is_available ? styles.cardDisabled : ""}`}
              >
                <div className={styles.cardHeader}>
                ...
                <div className={styles.meta}>
                  ...
                  <span
                    className={
                      t.is_available ? styles.availYes : styles.availNo
                    }
                  >
                ...
                <div className={styles.cardActions}>
                  <button
                    className={styles.smallBtn}
                    onClick={() => toggleTableAvail(t)}
                  >
                    {t.is_available ? "Mark Unavailable" : "Mark Available"}
                  </button>
                  <button
                    className={styles.smallBtn}
                    onClick={() => editTable(t)}
